...
class Robot():
	...
	def __init__(self, motorD, motorE, garra, sensorD, sensorE, sensorM, sensorU, R, r, dRodas):
		...
		self.motorG = LargeMotor('out' + garra)
	...
	def saindoPista(self): #Funcao que verifica se esta saindo da pista
	...
	def alinhaCor(self, cor, corAlinhamento, sensorEntrada, sensorAjuste, velocidadeMotorEntrada, velocidadeMotorAjuste, updateCor = False, garra = False):
		if self.ruido(cor, sensorEntrada, 5):
			if self.ruido(cor, sensorEntrada, 40):
				print("Cor para alinhamento: ", corAlinhamento)
				print("Cor atual: ", cor)
				while sensorEntrada.value() != corAlinhamento:
					print("Valor do sensor de entrada: ", sensorEntrada.value())
				...
				while sensorAjuste.value() not in cor:
					if self.corAtual == 2:
						...
						break
					print("Valor do sensor de ajuste: ", sensorAjuste.value())
					print('SensorE: ', self.sensorE.value())
					print('SensorD: ', self.sensorD.value())
					self.andarTempo(-velocidadeMotorEntrada, velocidadeMotorAjuste, 100)
					# if not garra:
					# 	if self.saindoPista():
					# 		return True
	...
	def verificaCorSensor(self, sensor, setCor = False):
		sensor.mode = 'RGB_RAW'

		r = self.sensor.value(0)
		g = self.sensor.value(1)
		b = self.sensor.value(2)
	...
	def voltaPraca(self):
		...
		while True:
			...
			if self.sensorD.value() in [3]:
				while not self.alinhaCor([3], 6, self.sensorD, self.sensorE, 0, 300, True):
					continue
				break

			if self.sensorE.value() in [3]:
				while not self.alinhaCor([3], 6, self.sensorE, self.sensorD, 300, 0, True):
					continue
				break
	...
	def modoAtaque(self):
		...
		while not self.garraOcupada: #and self.detectou == True: #se a garra estiver ocupada ele não tenta detectar
			...
			if self.detecta(): #se ele detecta ele faz teste de ruído e gira
				self.medeHipotenusaBoneco()
				print(self.sensorU.value())
				if self.sensorU.value() < 120:
					self.andarRoda("d",-40)
					self.andarRoda("e",-40)
			...
			while not (self.sensorE.value() in [0,1,7] or self.sensorD.value() in [0,1,7]): #enquanto o valor dos sensores não chegar em preto ou marrom/unknown ele anda
				self.andarTempo(100, 100, 100)
				print(self.sensorE.value(),self.sensorD.value())
			...
			if self.detecta():
				self.medeHipotenusaBoneco()
				#self.andarRotacao(111.1,111.1, 200,True)
	...
	def bateParede(self):
		...
		for i in range(80):
			self.andarTempo(400, 400, 100)
			if (self.motorE.speed < 370) and i > a:
				if self.motorE.speed < self.motorD.speed:
					e += 1
					bol = False
				else:
					d += 1
			...
			if (self.motorD.speed < 370) and i > a:
				if self.motorE.speed < self.motorD.speed:
					e += 1
					bol = False
				else:
